      ...
      for (int groupColId = -1; groupColId < groupColNum; groupColId++) {
        ...
        for (int i = 0; i < groupRowSize; i++) {
          for (int j = 0; j < groupColSize; j++) {
            ...
            BlockLocation[] blkLocations = fs.getFileBlockLocations(groupFilePaths[j + i * groupColSize], 0, 1);
        ...
        for (Map.Entry<String, Integer> stringIntegerEntry : tempList) {
          if (stringIntegerEntry.getValue() == groupColSize * groupRowSize) {
        ...
        splits.add(new FileSplitGroup(groupFilePaths, 1, hosts.toArray(new String[0]), splitId, cellRowSize,
