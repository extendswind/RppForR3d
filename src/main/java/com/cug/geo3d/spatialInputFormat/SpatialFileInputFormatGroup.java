...
//    long maxSize = getMaxSplitSize(job);
    ...
    for (int i = 0; i < cellRowNum - 1; i++) {
      for (int j = 0; j < cellColNum - 1; j++) {
        ...
        for (int ii = 0; ii < 2; ii++) {
          for (int jj = 0; jj < 2; jj++) {
            String cellName = spatialFilepath + SpatialConstant.GRID_INDEX_PREFIX + "_" +
...
//            FileStatus fileStatus = new FileStatus();//fs.getFileStatus(new Path(cellName));
        ...
        int splitId = j + i * cellColNum;
        splits.add(new EdgeFileSplit(edgeFilePaths, 1, hosts.toArray(new String[0]),
                splitId, cellRowNum, cellColNum, cellRowSize, cellColSize));
